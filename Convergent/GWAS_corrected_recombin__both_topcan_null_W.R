...
tmp_add <- "/data/users/mjahani/GWAS_conver/corrected/temporary_nullW/"
rslt_add <- "/data/users/mjahani/Convergent/GWAS_corrected/"
...
fread(paste0(tmp_add,Taxa_var[i]),showProgress = F) %>% 
  select(Value=V3,Taxa_var=V5) %>% mutate(Perc=quantile(Value, 0.01)) %>% distinct(Taxa_var, .keep_all = T) %>% select(-Value)
}
...
SNP_count <- foreach(i=1:nrow(percentile), .combine='rbind', .errorhandling='stop') %dopar% {
  fread(paste0(tmp_add,percentile[i,1]),showProgress = F) %>% 
    select(Taxa_var=V5,Value=V3,window5=V4) %>%
    ...
    mutate(quantile=cut(recomb_rate, 
                        ...
                        include.lowest=TRUE)) %>%
    select(-recomb_rate)
  #system(paste0("rm ",tmp_add,percentile[i,1]))
}
...
foreach(i=1:nrow(percentile), .combine='rbind', .errorhandling='stop') %dopar% {
  fread(paste0(tmp_add,percentile[i,1]),showProgress = F) %>%
    separate(V5,into=c("Taxa","Variable"),sep=":",extra="warn",remove = F) %>%
    select(Taxa,Variable,ID=V2,Value=V3,window5=V4,Taxa_var=V5) %>%
    left_join(.,SNP_count) %>%
...
"/data/mojtaba/GWAS_corrected/combination/"-> tmp_add
#copy these two files to server and change them below
"/data/mojtaba/ID_window_final" -> a
"/data/mojtaba/5kbwindow_recombination" -> b
#directory to save the results
"/data/mojtaba/GWAS_corrected/result/"-> result_directory
...
for (i in 1:nrow(combination_Taxa)) {#loop through Taxas
  ...
  for (z in 1:length(quantile_list)) {
    ...
    for (j in 1:length(variable_list)) {#loop through variables
      ...
      fwrite(Final_result, file = paste0(result_directory,"GWAScorrected_nullw_result_recom_bin"), append = T,sep = "\t")
      rm(nulldist,Result,temporary_result,Emp_result,Final_result,N_window)}
        ...
        data.frame(variable=variable_list[j],quantile=quantile_list[z],Taxa1=combination_Taxa[i,1],Taxa2=combination_Taxa[i,2]) %>%
          fwrite(file = paste0(result_directory,"zero_top_GWAScorrected"), append = T,sep = "\t")
...
all_vars<-read.table(paste0(result_directory,"GWAScorrected_nullw_result_recom_bin"), header = TRUE,sep="\t")
...
write.table(out_res,paste0(result_directory,"GWAScorrected_nullw_result_recom_bin_FDR"),col.names= TRUE, row.names = FALSE,sep="\t")
...
#copy this directory in server and change the addredd below
"/data/users/mjahani/Convergent/GWAS_corrected/files/combination/"-> tmp_add
"/moonriseNFS/Null_W_Result/files/ID_window_final" -> annot
"/moonriseNFS/Null_W_Result/dbinom_binning/GWAS_corrected/"-> result_directory
...
foreach(j= 1:length(var), .combine='rbind' , .errorhandling='stop') %dopar% {
  dbinom %>%
    ...
    fwrite(paste0(result_directory,"dbinom_score_GWAS_corrected","_",var[j]),sep = "\t",na = "NA",quote = F)
}


library(tidyverse)
library(data.table)
fread("/data/mojtaba/GWAS_corrected/result/GWAScorrected_nullw_result_recom_bin",sep="\t") %>%
  distinct(Taxa1,Taxa2,Variable,quantile) %>% nrow()/ 3010
